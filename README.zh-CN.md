...
你已经登录了微博、知乎、B站、小红书、Twitter、GitHub、LinkedIn — ping-browser 让 AI Agent **直接用你的登录态**。

```bash
ping-browser site twitter/search "AI agent"       # 搜索推文
ping-browser site zhihu/hot                        # 知乎热榜
ping-browser site arxiv/search "transformer"       # 搜论文
ping-browser site eastmoney/stock "茅台"            # 实时股票行情
ping-browser site boss/search "AI 工程师"           # 搜职位
ping-browser site wikipedia/summary "Python"       # 维基百科摘要
ping-browser site youtube/transcript VIDEO_ID      # YouTube 字幕全文
ping-browser site stackoverflow/search "async"     # 搜 StackOverflow
```

**36 个平台，103 个命令，全部用你真实浏览器的登录态。** [完整列表 →](https://github.com/sipingme/bb-sites)

## 核心理念

互联网是为浏览器构建的。AI Agent 一直试图通过 API 访问它 — 但 99% 的网站不提供 API。

ping-browser 翻转了这个逻辑：**不是让网站适配机器，而是让机器使用人的界面。** adapter 在你的浏览器 tab 里跑 `eval`，用你的 Cookie 调 `fetch()`，或者直接调用页面的 webpack 模块。网站以为是你在操作。因为**就是你**。

| | Playwright / Selenium | 爬虫库 | ping-browser |
|---|---|---|---|
...
```bash
ping-browser site update        # 拉取社区适配器
ping-browser site recommend     # 看看哪些和你的浏览习惯匹配
ping-browser site zhihu/hot     # 开搞
```

### OpenClaw（无需安装扩展）

如果你使用 [OpenClaw](https://openclaw.ai)，ping-browser 可以直接通过 OpenClaw 内置浏览器运行，不需要额外安装 Chrome 扩展或 daemon：
...
ClawHub Skill: [ping-browser-openclaw](https://clawhub.ai/yan5xu/ping-browser)

### Chrome 扩展（独立模式）

不使用 OpenClaw 时（Claude Code MCP、独立 CLI）需要安装扩展：

1. 从 [Releases](https://github.com/sipingme/ping-browser/releases/latest) 下载 zip
...
社区驱动，通过 [bb-sites](https://github.com/sipingme/bb-sites) 维护。每个命令一个 JS 文件。
...
没有 ping-browser，AI Agent 的世界是：**文件系统 + 终端 + 少数有 API key 的服务。**

有了 ping-browser：**文件系统 + 终端 + 整个互联网。**
...
ping-browser snapshot -i                # 可访问性树
ping-browser click @3                   # 点击元素
ping-browser fill @5 "hello"            # 填写输入框
ping-browser eval "document.title"      # 执行 JS
ping-browser fetch URL --json           # 带登录态的 fetch
ping-browser network requests --with-body --json  # 抓包
ping-browser screenshot                 # 截图
```
...
ping-browser site info xueqiu/stock   # 查看 adapter 参数、示例、域名
```
...
```bash
ping-browser daemon --host 127.0.0.1    # 仅 IPv4（解决 macOS IPv6 问题）
ping-browser daemon --host 0.0.0.0      # 监听所有网卡（用于 Tailscale / ZeroTier 跨机器访问）
```
...
AI Agent (Claude Code, Codex, Cursor 等)
       │ CLI 或 MCP (stdio)
       ▼
ping-browser CLI ──HTTP──▶ Daemon ──SSE──▶ Chrome 扩展
                                              │
                                              ▼ chrome.debugger (CDP)
                                         你的真实浏览器
```
