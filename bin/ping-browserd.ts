...
const PROVIDER_NAME_PREFIX = "ping-browserd";
...
const CDP_PORT_FILE = join(homedir(), ".ping-browser", "browser", "cdp-port");
...
async function launchManagedBrowser(port: number): Promise<boolean> {
  ...
  if (!executable) {
    console.log("[ping-browserd] No browser executable found, cannot auto-launch");
    return false;
  }
  const userDataDir = join(homedir(), ".ping-browser", "browser", "chrome-data");
  await mkdir(userDataDir, { recursive: true });
  console.log(`[ping-browserd] Launching browser: ${path.basename(executable)} on port ${port}`);
  try {
  ...
  while (Date.now() < deadline) {
    if (await canConnect(port)) {
      await mkdir(join(homedir(), ".ping-browser", "browser"), { recursive: true });
      await writeFile(join(homedir(), ".ping-browser", "browser", "cdp-port"), String(port), "utf8");
      console.log(`[ping-browserd] Browser launched, CDP ready on port ${port}`);
      return true;
    }
    await new Promise(r => setTimeout(r, 300));
  }
  console.log("[ping-browserd] Browser launched but CDP not ready within timeout");
  return false;
}
...
async function ensureCdp(): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    ws.onopen = () => {
      cdpSocket = ws;
      console.log(`[ping-browserd] CDP connected to ${wsUrl}`);
      resolve();
    };
    ...
    ws.onclose = () => {
      ...
      cdpSessionId = null;
      cdpTargetId = null;
      console.error("[ping-browserd] CDP disconnected");
    };
  });
  ...
  cdpTargetId = page.id;
  console.log(`[ping-browserd] Attached to target: ${page.title} (${page.url})`);
}
...
function printUsage(): void {
  console.log(`Usage: bun run bin/ping-browserd.ts [--pinix <url>] [--name <name>]
    ...
    payload: {
      ...
      value: {
        ...
        error: {
          ...
          message: "manage operations are not supported by ping-browserd",
        },
      },
    },
  };
...
class PinixBridge {
  ...
  private connect(): void {
    ...
    this.runStream()
      .catch((error) => {
        ...
        console.error(`[ping-browserd] Provider stream error: ${formatError(error)}`);
        this.scheduleReconnect();
      });
  }

  private async runStream(): Promise<void> {
    console.log(`[ping-browserd] Connecting to ${this.options.pinixUrl}`);
      ...
      for await (const message of responseStream) {
        ...
        switch (message.payload.case) {
          case "registerResponse": {
            ...
            this.clearReconnectTimer();
            console.log(`[ping-browserd] Connected to pinixd at ${this.options.pinixUrl}`);
            console.log(
              `[ping-browserd] Registered clip "${this.options.name}" via provider "${getProviderName(this.options.name)}" with commands: ${CAPABILITIES.join(", ")}`,
            );
            break;
          }
          case "invokeCommand": {
  ...
  private handleInvokeInput(message: InvokeInputPayload): void {
    ...
    if (requestId && (hasData || message.done === true)) {
      console.warn(`[ping-browserd] Ignoring InvokeInput for unary browser command: ${requestId}`);
    }
  }
  ...
  private send(queue: AsyncMessageQueue<ProviderMessage>, message: ProviderMessage): void {
    try {
      ...
      if (!this.stopped) {
        console.error(`[ping-browserd] Failed to send provider message: ${formatError(error)}`);
      }
    }
  }
  ...
  private scheduleReconnect(): void {
    ...
    console.log(`[ping-browserd] Reconnecting in ${RECONNECT_DELAY_MS}ms`);
...
function installProcessHandlers(bridge: PinixBridge): void {
  ...
  process.on("unhandledRejection", (reason) => {
    console.error(`[ping-browserd] Unhandled rejection: ${formatError(reason)}`);
  });
  process.on("uncaughtException", (error) => {
    console.error(`[ping-browserd] Uncaught exception: ${formatError(error)}`);
  });
}
...
try {
  ...
  console.error(`[ping-browserd] ${formatError(error)}`);
  process.exit(1);
}
