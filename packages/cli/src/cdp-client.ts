...
import type { Request, Response, ResponseData, TabInfo, SnapshotData, RefInfo, NetworkRequestInfo, ConsoleMessageInfo, JSErrorInfo, TraceEvent, TraceStatus } from "@ping-browser/shared";
import { COMMAND_TIMEOUT } from "@ping-browser/shared";
...
function getContextFilePath(host: string, port: number): string {
  ...
  return path.join(os.tmpdir(), `ping-browser-cdp-context-${safeHost}-${port}.json`);
}
...
function getRefsFilePath(targetId: string): string {
  return path.join(os.tmpdir(), `ping-browser-refs-${targetId}.json`);
}
