...
const HELP_TEXT = `
ping-browser - AI Agent 浏览器自动化工具

安装：
  npm install -g ping-browser

提示：大多数数据获取任务请直接使用 site 命令，无需手动操作浏览器：
  ping-browser site list                    查看所有可用命令
  ping-browser site twitter/search "AI"     示例：搜索推文
  ping-browser site xueqiu/hot-stock 5      示例：获取人气股票

用法：
  ping-browser <command> [options]
...
开始使用：
  ...
  star                         ⭐ Star ping-browser on GitHub
    ...
    switch (parsed.command) {
      case "open": {
        ...
        if (!url) {
          ...
          console.error("用法：ping-browser open <url> [--tab current|<tabId>]");
          process.exit(1);
        }
        // 解析 --tab 参数
      ...
      case "click": {
        ...
        if (!ref) {
          ...
          console.error("用法：ping-browser click <ref>");
          console.error("示例：ping-browser click @5");
          process.exit(1);
        }
      ...
      case "hover": {
        ...
        if (!ref) {
          console.error("错误：缺少 ref 参数");
          console.error("用法：ping-browser hover <ref>");
          console.error("示例：ping-browser hover @5");
          process.exit(1);
        }
      ...
      case "check": {
        ...
        if (!ref) {
          console.error("错误：缺少 ref 参数");
          console.error("用法：ping-browser check <ref>");
          console.error("示例：ping-browser check @5");
          process.exit(1);
        }
      ...
      case "uncheck": {
        ...
        if (!ref) {
          console.error("错误：缺少 ref 参数");
          console.error("用法：ping-browser uncheck <ref>");
          console.error("示例：ping-browser uncheck @5");
          process.exit(1);
        }
      ...
      case "fill": {
        ...
        if (!ref) {
          console.error("错误：缺少 ref 参数");
          console.error("用法：ping-browser fill <ref> <text>");
          console.error('示例：ping-browser fill @3 "hello world"');
          process.exit(1);
        }
        if (text === undefined) {
      ...
      case "type": {
        ...
        if (!ref) {
          console.error("错误：缺少 ref 参数");
          console.error("用法：ping-browser type <ref> <text>");
          console.error('示例：ping-browser type @3 "append text"');
          process.exit(1);
        }
        if (text === undefined) {
          console.error("错误：缺少 text 参数");
      ...
      case "select": {
        ...
        if (!ref) {
          console.error("错误：缺少 ref 参数");
          console.error("用法：ping-browser select <ref> <value>");
          console.error('示例：ping-browser select @4 "option1"');
          process.exit(1);
        }
        if (value === undefined) {
      ...
      case "eval": {
        ...
        if (!script) {
          ...
          console.error("用法：ping-browser eval <script>");
          console.error('示例：ping-browser eval "document.title"');
          process.exit(1);
        }
      ...
      case "get": {
        ...
        if (!attribute) {
          ...
          console.error("用法：ping-browser get <text|url|title> [ref]");
          console.error("示例：ping-browser get text @5");
          console.error("      ping-browser get url");
          process.exit(1);
        }
      ...
      case "wait": {
        ...
        if (!target) {
          ...
          console.error("用法：ping-browser wait <ms|@ref>");
          console.error("示例：ping-browser wait 2000");
          console.error("      ping-browser wait @5");
          process.exit(1);
        }
      ...
      case "press": {
        ...
        if (!key) {
          ...
          console.error("用法：ping-browser press <key>");
          console.error("示例：ping-browser press Enter");
          console.error("      ping-browser press Control+a");
          process.exit(1);
        }
      ...
      case "scroll": {
        ...
        if (!direction) {
          ...
          console.error("用法：ping-browser scroll <up|down|left|right> [pixels]");
          console.error("示例：ping-browser scroll down");
          console.error("      ping-browser scroll up 500");
          process.exit(1);
        }
      ...
      case "frame": {
        ...
        if (!selectorOrMain) {
          ...
          console.error("用法：ping-browser frame <selector>");
          console.error('示例：ping-browser frame "iframe#editor"');
          console.error("      ping-browser frame main");
          process.exit(1);
        }
      ...
      case "dialog": {
        ...
        if (!subCommand) {
          ...
          console.error("用法：ping-browser dialog <accept|dismiss> [text]");
          console.error("示例：ping-browser dialog accept");
          console.error('      ping-browser dialog accept "my input"');
          console.error("      ping-browser dialog dismiss");
          process.exit(1);
        }
      ...
      case "trace": {
        ...
        if (!subCmd || !['start', 'stop', 'status'].includes(subCmd)) {
          ...
          console.error("用法：ping-browser trace <start|stop|status>");
          console.error("示例：ping-browser trace start");
          console.error("      ping-browser trace stop");
          console.error("      ping-browser trace status");
          process.exit(1);
        }
      ...
      case "history": {
        ...
        if (!subCmd || !['search', 'domains'].includes(subCmd)) {
          console.error("错误：缺少或无效的子命令");
          console.error("用法：ping-browser history <search|domains> [query] [--days <n>]");
          console.error("示例：ping-browser history search github");
          console.error("      ping-browser history domains --days 7");
          process.exit(1);
        }
      ...
      case "fetch": {
        ...
        if (!fetchUrl) {
          ...
          console.error("  Usage: ping-browser fetch <url> [--json] [--method POST] [--body '{...}']");
          console.error("  Example: ping-browser fetch https://www.reddit.com/api/me.json --json");
          process.exit(1);
        }
        // 解析 fetch 特有选项
      ...
      case "star": {
        ...
        const repos = ["sipingme/ping-browser", "sipingme/bb-sites"];
      ...
      case "guide": {
        console.log(`How to turn any website into a ping-browser site adapter
=======================================================

1. REVERSE ENGINEER the API
   ping-browser network clear --tab <tabId>
   ping-browser refresh --tab <tabId>
   ping-browser network requests --filter "api" --with-body --json --tab <tabId>
...
4. TEST it
   Save to ~/.ping-browser/sites/platform/command.js (private, takes priority)
...
5. CONTRIBUTE
   ...
   Option B (without gh CLI, using ping-browser itself):
...
Private adapters:  ~/.ping-browser/sites/<platform>/<command>.js
Community:         ~/.ping-browser/bb-sites/ (via ping-browser site update)
Full guide:        https://github.com/sipingme/bb-sites/blob/main/SKILL.md`);
        break;
      }

      default: {
        console.error(`错误：未知命令 "${parsed.command}"`);
        console.error("运行 ping-browser --help 查看可用命令");
        process.exit(1);
      }
    }
  } catch (error) {
