 ...
 *
 * 用法：
 *   ping-browser fetch <url> [options]
 *   ping-browser fetch https://www.reddit.com/api/me.json
 *   ping-browser fetch /api/me.json                     # 相对路径，用当前 tab 的 origin
 *   ping-browser fetch https://www.reddit.com/... --json
 *   ping-browser fetch https://x.com/... --method POST --body '{"query":"..."}'
 *
 * 本质：curl，但带浏览器登录态。
 */
...
export async function fetchCommand(
  ...
  if (!url) {
    throw new Error(
      "缺少 URL 参数\n" +
      "  用法: ping-browser fetch <url> [--json] [--method POST] [--body '{...}']\n" +
      "  示例: ping-browser fetch https://www.reddit.com/api/me.json --json"
    );
  }
