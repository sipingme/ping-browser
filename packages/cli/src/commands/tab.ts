 ...
 * tab 命令 - 标签页管理
 * 用法：
 *   ping-browser tab                    列出所有标签页
 *   ping-browser tab new [url]          新建标签页
 *   ping-browser tab <n>                切换到第 n 个标签页（按 index）
 *   ping-browser tab close [n]          关闭标签页（按 index）
 *   ping-browser tab select --id <id>   切换到指定 tabId 的标签页
 *   ping-browser tab close --id <id>    关闭指定 tabId 的标签页
 */
...
function parseTabSubcommand(args: string[], rawArgv?: string[]): {
  ...
  if (first === "select") {
    ...
    throw new Error("tab select 需要 --id 参数，用法：ping-browser tab select --id <tabId>");
  }
