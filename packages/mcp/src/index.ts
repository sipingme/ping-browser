...
import { DAEMON_BASE_URL, COMMAND_TIMEOUT, generateId } from "@ping-browser/shared";
...
const EXT_HINT = [
  ...
  "",
  "1. Download extension: https://github.com/sipingme/ping-browser/releases/latest",
...
async function sendCommand(request: Request): Promise<Response> {
  ...
  try {
    ...
    clearTimeout(timeoutId);
    return { id: request.id, success: false, error: "Failed to start daemon. Run manually: ping-browser daemon" };
  }
}
...
function formatSiteCliError(value: unknown, stderr: string, stdout: string): string {
  ...
  return fallback || "ping-browser site command failed";
}
...
const server = new McpServer(
  { name: "ping-browser", version: __BB_BROWSER_VERSION__ },
  { instructions: `ping-browser lets you control the user's real Chrome browser — with their login state, cookies, and sessions.
...
- browser_close_all: Close tabs opened by ping-browser during the current MCP session
...
To create a new site adapter, run: ping-browser guide` },
);
...
server.tool(
  "browser_close_all",
  "Close tabs opened by ping-browser during the current MCP session",
  {},
  async () => {
