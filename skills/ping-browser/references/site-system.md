...
ping-browser site search <query>

# 运行 adapter（简写，推荐）
ping-browser site <name> [args...]

# 运行 adapter（完整写法）
ping-browser site run <name> [args...]

# 更新社区 adapter 库（从 github.com/sipingme/bb-sites 拉取）
...
# 命名参数（--flag value 格式）
ping-browser site github/pr-create sipingme/bb-sites --title "feat: ..." --head "user:branch"

# 混合使用
ping-browser site twitter/search "AI agent" --count 20
```
...
│   └── platform/
│       └── command.js
└── bb-sites/           # 社区 adapter（通过 ping-browser site update 拉取）
    └── platform/
        └── command.js
```
...
<!-- 证据来源：README.md "36 platforms, 103 commands"，具体 adapter 数量以 ping-browser site list 实际输出为准 -->
...
```bash
# 搜索技术话题
ping-browser site twitter/search "Claude Code"
ping-browser site reddit/search "local LLM"
ping-browser site hackernews/top

# 查看热榜
ping-browser site zhihu/hot
ping-browser site weibo/hot
ping-browser site v2ex/hot

# 查询股票
ping-browser site eastmoney/stock "茅台"
```
...
```bash
# GitHub 操作
ping-browser site github/repo anthropics/claude-code
ping-browser site github/issues owner/repo

# 查包信息
ping-browser site npm/package ping-browser
ping-browser site pypi/package requests

# 搜索技术问题
ping-browser site stackoverflow/search "async await error handling"
```
...
```bash
# 获取 YouTube 字幕
ping-browser site youtube/transcript dQw4w9WgXcQ

# 获取 Reddit 帖子完整内容
ping-browser site reddit/thread https://www.reddit.com/r/LocalLLaMA/comments/...

# 论文搜索
ping-browser site arxiv/search "transformer attention mechanism"
```
...
```bash
ping-browser site zhihu/hot --json
ping-browser site twitter/search "AI" --json
```

## 更多信息

- 创建自定义 adapter：参见 [adapter-development.md](adapter-development.md)
- 社区 adapter 仓库：https://github.com/sipingme/bb-sites
